...
父盒子设置为flex布局之后，子元素的float，clear和vertical-align 属性失效
...
![67101792498](../../AppData/Roaming/Typora/qianduan/JavaScript基础笔记和作业/JavaScript基础笔记和作业/JavaScript基础第二天/03-笔记/assets/1671017924981.png)
...
![universe](../../AppData/Roaming/Typora/qianduan/JavaScript基础笔记和作业/JavaScript基础笔记和作业/JavaScript基础第三天/02-笔记/assets/universe.gif)

实际上 JavaScript 中任何一种循环语句都支持循环的嵌套，如下代码所示：

![64791826139](../../AppData/Roaming/Typora/qianduan/JavaScript基础笔记和作业/JavaScript基础笔记和作业/JavaScript基础第三天/02-笔记/assets/1647918261399.png)
...
~~~
 ...
 ![64791867895](../../AppData/Roaming/Typora/qianduan/JavaScript基础笔记和作业/JavaScript基础笔记和作业/JavaScript基础第三天/02-笔记/assets/1647918678956.png)
...
![64791873467](../../AppData/Roaming/Typora/qianduan/JavaScript基础笔记和作业/JavaScript基础笔记和作业/JavaScript基础第三天/02-笔记/assets/1647918734677.png)
...
![array](../../AppData/Roaming/Typora/qianduan/JavaScript基础笔记和作业/JavaScript基础笔记和作业/JavaScript基础第三天/02-笔记/assets/array.jpg)
...
```html
let price = +prompt('请输入商品价格')   price里面存的就是字符型了
```
...
![1676896351814](../../AppData/Roaming/AppData/Roaming/Typora/typora-user-images/1676896351814.png)
...
js对象是一组无需的相关属性和方法的集合，所有的事物都是对象，例如：字符串，数值，数组，函数等
...
<head>
    ...
    <script>
        ...
        // (3)方法冒号后面跟的是一个匿名函数
        // 2.使用对象
        //（1）调用对象属性 采用    对象名.属性名   . 我们理解为 的 
        console.log(obj.uname);
        //(2) 调用对象属性 第二种方法   对象名['属性名']  中括号加单引号
        console.log(obj['age']);
...
1.变量和属性的相同点：他们都是用来存储数据的
 	 ...
 	 变量 单独声明 并赋值 使的时候直接写变量名 单独存在
	  属性 在对象里面不需要声明的 使用的时候必须是  对象.属性

2.函数和方法的相同点 都是实现某种功能
	 函数是单独声明 并且调用的   调用：函数名()
	 方法 在对象里面 调用的时候  调用：  对象.方法()
...
<head>
    ...
    <title>Document</title>
    <script>
        var obj = new Object(); //创建了一个空的对象  new Obiect 注意大小写
        obj.uname = '张三';
        obj.age = 18;
        obj.sex = '男';
        obj.sayHi = function () {
...
**构造函数是一种特殊的函数，主要用来初始化对象，即为对象成员变量赋初始值，它总与new运算符一起使用，我们可以把对象中一些公共属性和方法提取出来，然后封装到这个函数里面。**
...
<head>
    <script>
        // 创建四大天王的对象    相同的属性：姓名，年龄，性别，相同的方法：都会唱歌
        //构造函数的语法格式
        function 构造函数名() {
            this.属性 = 值;        //this当前的意思
            this.方法 = function () { }
...
<head>
    ...
    <script>
        ...
        // console.log(typeof ldh);    结果是 对象 object
...
构造函数和对象
1.构造函数   抽象了对象的公共部分 封装到了函数里面，泛指的某一大类 
2.创建对象  特指 对象是一个具体的事物 
...
**2.this就会指向刚才创建的空对象**
**3.开始执行构造函数里面的代码 给空对象添加属性和方法**
**4.返回这个新对象(所以构造函数不需要return)**

### for...in语句遍历对象

for...in语句用于对数组或者对象属性进行循环操作。
...
![1677035396526](images/1677035396526.png)

```js
conlise.log(11 && 22)  //都是真，返回最后一个真值
conlise.log(11 || 22)  //输出第一个真值
```

```javascript
function fn(x,y) {    //x,y是undefined
	x = x || 0			//undefined 相当于false  逻辑或 返回第一个真值 所以x，y都是 0
    y = y || 0
    console.log(x+y)
}
	fn()
...
![1677037634154](images/1677037634154.png)
...
![1677038063228](images/1677038063228.png)
...
//1. 遍历对象 for in 
        ...
        //2.表遍历对象
        for (let k in obj) {
            console.log(k);  // 结果是 'uname' 'age' 'gender'（属性名）(字符串)
            // console.log(obj.uname);  // 找obj.uname
            // console.log(obj.k);  //  这里找的是 obj.k 而log里面没有k   underfined
            // console.log(obj.'uname');   相当于此
            // console.log(obj['uname']);
            console.log(obj[k]);
        }
~~~
...
```html
备注： const
	const 对象只是添加或者删除某些元素 没有改变原有对象的地址 不会报错
	给一个新对象 地址会改变 报错
	
```

![1677928314657](images/1677928314657.png)
...
1. querySelector   满足条件的**第一个**元素   ==结果是对象==
2. querySelectorAll  满足条件的元素集合 返回伪数组    ==数组== （有长度有索引号的数组  但是没有 pop() push() 等数组方法，想要得到里面的每一个对象，则需要遍历（for）的方式获得。）
3. 了解其他方式
   1. getElementById
...
<script>
  ...
  // intro.innerText = '<h4>嗨~ 我叫李雷！</h4>'  //innerText 不解析标签 只获取/修改文本
</script>
```
...
任何标签都有 `style` 属性，通过 `style` <!--属性可以动态更改网页标签的样式-->，如要遇到 `css` 属性中包含字符 `-` 时，要将 `-` 去掉并将其后面的字母改成大写，如 `background-color` 要写成 `box.style.backgroundColor`==小驼峰命名法==      ==带单位==
...
<body>
    ...
    <script>
        ...
        // 切换类   toggle（翻译：切换）  有就删除，没有就加上
        box.classList.toggle('one')
        
        
        calssList-contains() 包含某个类 如果包含就返回true 没有就返回 false
    </script>
</body>
...
* ==表单属性中==添加就有效果,移除就没有效果,==一律使用布尔值表示== 如果为true 代表添加了该属性 如果是false 代 
...
`setInterval` 是 JavaScript 中内置的函数，它的作用是间隔固定的时间自动重复执行另一个函数，也叫定时器函数。==间隔时间单位是毫秒==
...
```javascript
 <script>
        ...
        btn.addEventListener('click', function () {
            const num = Math.random()   // 局部作用域 有垃圾回收机制（变量不再使用 会被销毁）
            console.log(num);
        })
    </script>
```
...
![1678495202300](images/1678495202300.png)
...
```html
label.active 
选中lable便签 并且里面有class的值为‘active’
```
...
![1678610534390](images/1678610534390.png)
...
![event](../qianduan/JavaScript/JS案例/API/JavaScript APIs 素材/资料/web APIs第三天/02-笔记/assets/event.png)
...
![1678703425470](images/1678703425470.png)
...
document.documentElement.scrollTop  // 是html
~~~

<img src="images/1678706519461.png" alt="1678706519461" style="zoom:67%;" />

<img src="images/1678707726876.png" alt="1678707726876" style="zoom:80%;" />
...
![1678707869265](images/1678707869265.png)
...
| offsetWidth和offsetHeight   | 获得元素宽度和高度                       | 包含border、padding，滚动条等，不包含margin，只读         |
...
<body>
    ...
    <script>
        ...
        console.log(baby)  // dom 对象
        console.log(baby.parentNode)  // 父节点
        console.log(baby.parentNode.parentNode)  // 爷爷节点
    </script>
</body>
```
...
-  `append` 可插入Node节点或字符串，但 `appendChild` 只能插入Node节点 
...
```javascript
父元素.appendChild(要插入的元素)  //插入到父元素的最后末尾
父元素.insertBefore(要插入的元素,在哪个元素的前面)  
	// ul.children  得到数组  
	// ul.insertBefore(li, ul.children[0])  在ul的最前方插入
```
...
删除现有的 DOM 节点，也需要关注两个因素：首先由**父节点**删除子节点，其次是要删除哪个子节点。
...
children 属性 ----仅获得所有元素节点,返回的还是一个伪数组      父元素.children
...
增删都是操作数组，渲染数组，不是操作dom。

新建空白数组

阻止表单的默认行为 不让表单跳转	元素.preventDefault
...
删除数组，点击事件 写上自定义属性来用作下标

内容为空的话，弹出提示框，（判断方法是input框里都有name属性）
...
* 一旦执行栈中的所有同步任务执行完毕，系统就会按次序读取任务队列中的异步任务，于是被读取的异步任务结束等待 

状态，进入执行栈，开始执行。



![1679058424257](images/1679058424257.png)

![1679058719654](images/1679058719654.png)
...
<body>
  <script>
    ...
    // 1. 存储
    localStorage.setItem('age', 18)   // localStorage.setItem('键', 18)  键永远带引号
...
console.log(newArr)   // ['red颜色', 'pink颜色', 'blue颜色']  是数组
</script>
</body>
...
```javascript
本表格是对数组进行操作 数组也是对象
把数据变成JSON字符串存到本地刷新不会丢失 用 localStorage.setItem('data', JSON.stringify(initData))
把数据从本地拿出来（是字符串） 转成对象才能进行灵活操作  const arr = JSON.parse(localStorage.getItem('data'))
渲染是用map方法渲染 其特点是有返回值 用return 加模板字符串arr.map(function (ele, index) {return ``}
对象不能直接追加 要转成一个字符串 用 变量.join('')
新增的时候 arr一定要是数组对象 然后用push方式往里追加 ， 然后把最新的数组转为JSON字符串存入本地
删除用自定义id 和map的index绑定 arr.splice删除数组里的内容 最后再渲染页面 把最新的数组转为JSON 字符串存入本地
...
<img src="../qianduan/JavaScript/JS案例/API/JavaScript APIs 素材/资料/web APIs第七天实战/实战文档/assets/444.gif">
...
## 图片切换模块
 ...
 <img src="../qianduan/JavaScript/JS案例/API/JavaScript APIs 素材/资料/web APIs第七天实战/实战文档/assets/111.gif">



## 放大镜效果

<img src="../qianduan/JavaScript/JS案例/API/JavaScript APIs 素材/资料/web APIs第七天实战/实战文档/assets/555.gif">
...
### 点击模块
 ...
 <img src="../qianduan/JavaScript/JS案例/API/JavaScript APIs 素材/资料/web APIs第七天实战/实战文档/assets/666.gif">

### tab栏切换模块

<img src="../qianduan/JavaScript/JS案例/API/JavaScript APIs 素材/资料/web APIs第七天实战/实战文档/assets/777.gif">
...
数组解构和立即执行函数 要加分号  （以数组开头【2，3】.map   类型的前面要加分号隔开
...
<body>
  <script>
    ...
    function render({ data: myData }) {      //旧变量名：新变量名  冒号表示 什么值赋值给谁
...
function Goods(name, price, count) {
  ...
  //1.new Goods() 创建一个新的空对象
  //2.this.name 指向新的对象  
  //3.里面的参数根据实参变化 相当于 obj.name='佩奇' 
  //4.添加给新对象
...
2. `Object.assign` 静态方法创建新的对象   （合并一个多个对象到目标对象，并返回目标对象）
3. `Object.keys` 静态方法获取对象中所有属性    返回的是**数组**
4. `Object.values` 表态方法获取对象中所有属性值  返回的是**数组**
...
```javascript
        ...
        console.log(re); //['pjnk', 28]
--------
		const re1 = Object.keys(obj)
        console.log(re1); //['name', 'age']
...
// 案例
 <script>
        ...
        const total = arr.reduce((prev, current) => {
            // console.log(prev); // {name: '张三', salary: 10000} 对象 如果不写初始值 默认以数组的第一个元素为初始值 而当前第一个元素是对象
...
对象都会有一个属性 _ proto _ 指向构造函数的 prototype 原型对象，之所以我们对象可以使用构造函数 prototype 原型对象的属性和方法，就是因为对象有 _ _proto__ 原型的存在。
...
![1680226794092](images/1680226794092.png)
...
**只要是原型对象里面就有constructor指向创造自己的构造函数**
...
1. 节流和防抖的区别是？ 
   ...
   防抖：如果在 n 秒内又触发了事件，则会重新计算函数执行时 间 
...
<script>
        ...
        //在setTimeout 中是无法删除定时器的，因为定时器还在运作，所以使用timer=null而不是clearTimeout(timer)
    </script>
```
...
| 节流     | 单位时间内，频繁触发事件，只执行一次     | 高频时间：鼠标移动 mousemove ，页面尺寸缩放resize，滚动条scrool等待 |



![1680835150600](images/1680835150600.png)



![1680835168093](images/1680835168093.png)
